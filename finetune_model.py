...
# class FeedForward_MoE(FMoETransformerMLP):
#     def __init__(self, dim, num_expert, top_k, dropout=0., mult=4.):
#         super().__init__(num_expert=num_expert, d_model=dim, d_hidden=dim*mult*2, top_k=top_k,
#                 activation=QuickGELU())
#         print('init the fast moe')
# #         self.layer_norm = nn.LayerNorm(dim)
# #         self.dropout = nn.Dropout(dropout)

#     def forward(self, x):
#         core_out = super().forward(x)
...
#        self.mlp=FeedForward_MoE(d_model,12,2)
...
class CLIP(nn.Module):
    ...
    def criterion(self, key, query, temp, queue, steps, query_len=200):
        labels = torch.arange(key.shape[0])
        key = key / key.norm(dim=-1, keepdim=True)
        query = query / query.norm(dim=-1, keepdim=True)
        scores = torch.einsum('ab,cb->ac', key, query)/self.temp
    ...
    def encode_text(self, text, boolean=True):
    ...
    def forward_mlm(self, image, text, img_queue, steps, pad_id):
        sample_zh_inp, sample_zh, mlm_labels = text
        img_features = image
        _, zh_features = self.encode_text(sample_zh_inp, boolean=False)
        temp = self.logit_scale.exp()
        loss_1, img_queue = self.criterion(zh_features, img_features, temp, img_queue, steps)
        text_features, _ = self.encode_text(sample_zh, boolean=False)
   ...
   #     feature1,feature2=text_features1.mean(1),text_features2.mean(1)
    ...
    def forward(self, text, img_queue, steps):
        zh_features = self.bert_model(**text, output_hidden_states=True, return_dict=True).pooler_output
        zh_features=zh_features.view(-1,3,zh_features.shape[-1])
        z1,z2,z3=list(map(self.gather,[zh_features[:,0],zh_features[:,1],zh_features[:,2]]))
        key=torch.cat([z2,z3],dim=0)
        
        temp = self.logit_scale.exp()
        loss_1, img_queue = self.criterion(z1, key, temp, img_queue, steps,query_len=0)
        loss = loss_1
